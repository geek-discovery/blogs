...
* Les critères de *localisation* peuvent désormais reposer sur trois types. Chacun peut apporter des bénéfices et des limites :
...
- via la définition de ranges IP Publiques spécifiques : le gain principal est qu’une entreprise dispose de la possibilité de construire une table en liaison avec l’adressage IP (NAT) de leurs accès Internet.  Néanmoins la limite principale de ce type est complètement conditionné par l’architecture des accès Internet (*Offloading du traffic* en local sur les sites, solutions *SASE* qui ne dédient pas des adresses IP Publiques à leur clients …)
